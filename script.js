...
async function showDetails(day, month, year) {
  ...
  const response = await fetch("https://api.freeastrologyapi.com/panchang", {
    method: "POST",
    headers: {
  ...
  if (data.tithi) {
    details.innerHTML = `
      <h3>Panchangam for ${day}-${month}-${year}</h3>
      <p><strong>Tithi:</strong> ${data.tithi}</p>
      <p><strong>Nakshatra:</strong> ${data.nakshatra}</p>
      <p><strong>Rahukalam:</strong> ${data.rahu_kalam}</p>
    `;
  } else {
    details.innerHTML = "<p>Details not available. Please try another date.</p>";
  }
}
